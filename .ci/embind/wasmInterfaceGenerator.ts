...
export const generateWasmInterfaces = async (definitionFilepath: string, interfaceName: string, containerName: string, exportSpecifications: ExportSpecification[]) => {
    const definition : any = await readJsonFile(definitionFilepath)
    const interfaceDefinitions : InterfaceDefinition = interpretDefinition(definition)
    await exportSpecifications.forEach(async (specification: ExportSpecification) => {
        ...
        if (exportData) {
            let headerText = exportType == 'c++' ? '// This file was created automatically\n' : ''
            try {
                const template: string = specification.template ? await readTextFile(specification.template) : "";
            ...
            if (headerText)
            await writeOutputFile(specification.filepath, headerText + '\n' + exportData)
            else
...
const interpretDefinition = (definition : any) : InterfaceDefinition => {
    ...
    definition.objects?.forEach((element : ObjectDefinition) => {
        ...
        if (lowerType == 'enum') {
            ...
            fields.sort((a: FieldObject, b: FieldObject) => { return (a.name < b.name) ? -1 : ((a.name > b.name) ? 1 : 0) })
            valueObjects.push({
                'name': element.name,
            ...
            classElement.methods.forEach((item: MethodDefinition | PropertyDefinition) => {
                const lowerMethodType: string = item.type.toLocaleLowerCase()
                if (lowerMethodType == 'class_function') {
                    classFunctions.push({ 'name' : item.name, 'object': item as MethodDefinition})
                } else if (lowerMethodType == 'function' || lowerMethodType == 'method' || lowerMethodType == 'method_const') {
                    functions.push({ 'name' : item.name, 'object': item as MethodDefinition})
                } else if (lowerMethodType == 'smart_ptr_constructor') {
                    constructors.push({ 'name' : item.name, 'object': item as MethodDefinition})
                } else if (lowerMethodType == 'property' || lowerMethodType == 'class_property') {
                    properties.push({ 'name' : item.name, 'object': item as PropertyDefinition})
                }
            })
            ...
            functions.push({ 'name' : element.name, 'object': element as FunctionDefinition})
        }
    });
    enums.sort((a: EnumObject, b: EnumObject) => { return (a.name < b.name) ? -1 : ((a.name > b.name) ? 1 : 0) })
    valueObjects.sort((a: ValueObjectObject, b: ValueObjectObject) => { return (a.name < b.name) ? -1 : ((a.name > b.name) ? 1 : 0) })
    ...
    return {'enums': enums, 'valueObjects': valueObjects, 'vectors': vectors, 'maps': maps, 'templates': templates, 'classes': classes, 'functions': functions}
}

const cppFunctionSignature = (functionObject : FunctionObject) : string => {
    let templateSignature: string = '';
    if (functionObject.object.parameters?.every(parameter => parameter.cppType) ?? functionObject.object.cppType) {
        templateSignature += functionObject.object.cppType ?? 'void'
        functionObject.object.parameters?.forEach(parameter => templateSignature += `, ${parameter.cppType}`)
    }
    const signature : string = `emscripten::function${templateSignature ? `<${templateSignature}>` : ''}("${functionObject.object.name}", &${functionObject.object.cppInterface})`
    return signature
}

const cppSmartPointerConstructorSignature = (methodObject : MethodObject) : string => {
    let templateSignature: string = '';
    if (methodObject.object.parameters?.every(parameter => parameter.cppType) ?? methodObject.object.cppType) {
        templateSignature += methodObject.object.cppType ?? 'void'
        methodObject.object.parameters?.forEach(parameter => templateSignature += `, ${parameter.cppType}`)
    }
    const name = methodObject.object.cppInterface ? `smart_ptr_constructor`: 'smart_ptr';
    const methodArgument = methodObject.object.cppInterface ? `, &${methodObject.object.cppInterface}`: '';
    const signature : string = `.${name}${templateSignature ? `<${templateSignature}>` : ''}("${methodObject.object.name}"${methodArgument})`
    return signature
}
...
const cppClassFunctionSignature = (methodObject : MethodObject) : string => {
    ...
    const signature : string = `.class_function${templateSignature ? `<${templateSignature}>` : ''}("${methodObject.object.name}", &${methodObject.object.cppInterface})`
    return signature
}

const cppMethodSignature = (classObject: ClassDefinition, methodObject : MethodObject) : string => {
    let templateSignature: string = '';
    if (methodObject.object.parameters?.every(parameter => parameter.cppType) ?? methodObject.object.cppType) {
        templateSignature += methodObject.object.cppType ?? 'void'
        templateSignature += methodObject.object.type == 'function' ? `(${classObject.cppType}&` : `(${classObject.cppType}::*)(`
    ...
    const signature : string = `.function${templateSignature ? `<${templateSignature}>` : ''}("${methodObject.object.name}", &${methodObject.object.cppInterface})`
    return signature
}
...
const tsMethodSignature = (functionsDefinition : MethodObject) : string => {
    ...
    if (functionsDefinition.object.parameters) {
        for (let i = 0; i < functionsDefinition.object.parameters.length; i ++) {
            ...
            if (i < functionsDefinition.object.parameters.length-1) signature += ', '
        }
    }
...
const createTsInterface = (containerName: string, interfaceDefinitions : InterfaceDefinition) : string => {
    ...
    interfaceDefinitions.enums.forEach((element : EnumObject) => {
        ...
        if (element.values) {
            for (let i=0; i < element.values.length; ++i) {
                classInterfaceData += `    ${element.values[i].object.value}${i < element.values.length-1 ? ',' : ''}\n`
            }
        }
        classInterfaceData += `}\n\n`
    })
    ...
    interfaceDefinitions.valueObjects.forEach((element : ValueObjectObject) => {
        ...
        element.fields.forEach((fields : FieldObject) => {
            const access = (fields.object.cppGetter && !fields.object.cppSetter) ? 'readonly ' : ''
            classInterfaceData += `    ${access}${fields.object.name}${fields.object.optional ? '?' : ''}: ${fields.object.tsType || "any"};\n`
        })
        classInterfaceData += `}\n\n`
    })
...
const createCppInterface = (interfaceName: string, interfaceDefinitions : InterfaceDefinition) : string => {
    ...
    interfaceDefinitions.valueObjects.forEach((element : ValueObjectObject) => {
        ...
        element.fields.forEach((field : FieldObject) => {
            if (field.object.cppAttribute) {
                const templateSignature: string = field.object.cppType ? `<${element.object.cppType}, ${field.object.cppType}>`: ''
                ...
                let templateSignature: string = ''
                const className = field.object.cppClassType ? field.object.cppClassType : element.object.cppType
                if (field.object.perValue) {
                    templateSignature = field.object.cppType ? `<${field.object.cppType}(*)(const ${className}&), void(*)(${className}&, ${field.object.cppType})>`: ''
                } else {
                    templateSignature = field.object.cppType ? `<${field.object.cppType}(*)(const ${className}&), void(*)(${className}&, const ${field.object.cppType}&)>`: ''
                }
    ...
    let classInterfaceData : string = ''
    interfaceDefinitions.classes.forEach((element : ClassObject) => {
        const baseClassSignature = element.object.cppBaseClass ? `, ${element.object.cppBaseClass}` : ''
        ...
        element.constructors.forEach((constructorDefinition : MethodObject) => {
            ...
            classInterfaceData += `        ${cppSmartPointerConstructorSignature(constructorDefinition)}\n`
            if (constructorDefinition.object.preprocessorDefinition) {
        ...
        element.classFunctions.forEach((classFunctionsDefinition : MethodObject) => {
            ...
            classInterfaceData += `        ${cppClassFunctionSignature(classFunctionsDefinition)}\n`
            if (classFunctionsDefinition.object.preprocessorDefinition) {
                classInterfaceData += '#endif\n'
            }
        })
        element.functions.forEach((methodDefinition : MethodObject) => {
            ...
            classInterfaceData += `        ${cppMethodSignature(element.object, methodDefinition)}\n`
            if (methodDefinition.object.preprocessorDefinition) {
                classInterfaceData += '#endif\n'
            }
        })
        element.properties.forEach((propertyDefinition : PropertyObject) => {
            ...
            const bindingFunction = propertyDefinition.object.type === 'class_property' ? 'class_property' : 'property'
    ...
    let cppInterfaceData : string = interfaceName ? `EMSCRIPTEN_BINDINGS(${interfaceName}) {\n` : ''
    cppInterfaceData += `
...
const createMd = (interfaceName: string, interfaceDefinitions : InterfaceDefinition) : string => {
    let docsData : string = ''
    ...
    if (interfaceDefinitions.vectors.length > 0 || interfaceDefinitions.maps.length > 0 || interfaceDefinitions.templates.length > 0) {
        docsData += `## Containers\n\n`
    ...
    if (interfaceDefinitions.valueObjects.length > 0) {
        ...
        interfaceDefinitions.valueObjects.forEach((element : ValueObjectObject) => {
            ...
            if (element.fields !== undefined && element.fields.length > 0) {
                element.fields.forEach((fields : FieldObject) => {
                    docsData += `- \`${fields.object.name}${fields.object.optional ? '?' : ''}: ${fields.object.tsType || "any"}\`\n`
                })
                docsData += `\n`
            }
        });
    ...
    if (interfaceDefinitions.classes.length > 0) {
        ...
        interfaceDefinitions.classes.forEach((element : ClassObject) => {
            ...
            if (element.classFunctions !== undefined && element.classFunctions.length > 0) {
            ...
            if (element.properties !== undefined && element.properties.length > 0) {
                docsData += `properties:\n\n`
                element.properties.forEach((propertyDefinition : PropertyObject) => {
                    docsData += `- \`${propertyDefinition.object.name}: ${propertyDefinition.object.tsType || "any"}\`\n`
                })
                docsData += `\n`
            }
        });
